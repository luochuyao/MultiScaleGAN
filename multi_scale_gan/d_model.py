...
class DiscriminatorModel:
    def __init__(self, session, summary_writer, height, width, scale_conv_layer_fms,
                 scale_kernel_sizes, scale_fc_layer_sizes,is_w = False):
        """
    ...
    def discriminator_vars(self):
        return [var for var in tf.global_variables() if 'scale_net_' in var.name]
    ...
    def define_graph(self):
        ...
        with tf.name_scope('discriminator'):
            ...
            with tf.name_scope('training'):
                ...
                self.global_loss = adv_loss(self.scale_preds, self.labels,self.is_w)
                self.global_step = tf.Variable(0.0, trainable=False, name='global_step')

                with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                    if self.is_w == True:
                        self.optimizer = tf.train.RMSPropOptimizer(self.c .LRATE_D, name='optimizer')
                    else:
                ...
                self.clip = [v.assign(tf.clip_by_value(v, -0.01, 0.01)) for v in self.discriminator_vars]
    ...
    def build_feed_dict(self, input_frames, gt_output_frames, generator):
        ...
        for scale_num in range(self.num_scale_nets):
            ...
            scaled_gt_output_frames = np.empty([batch_size, scale_net.height, scale_net.width, 1])
            for i, img in enumerate(gt_output_frames):
                ...
                resized_frame = resize(sknorm_img, [scale_net.height, scale_net.width, 1])
    ...
    def train_step(self, batch, generator):
        ...
        if self.is_w:
            self.sess.run(self.clip,feed_dict=feed_dict)
