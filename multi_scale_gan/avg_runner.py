...
class AVGRunner:
    ...
    def train(self):
        ...
        for i in range(self.num_steps):
            if c.ADVERSARIAL:
                ...
                batch = self.train_data_itertor.sample(c.BATCH_SIZE)
                batch[batch>80.0] = 0
                batch[batch<15.0] = 0
            ...
            if self.global_step % c.TEST_FREQ == 0:
                ...
                if valid_res < self.best_valid_res:
                    self.saver.save(self.sess,
                                    c.MODEL_SAVE_DIR + 'model_best.ckpt', )
                    print('Model improve')
    ...
    def test(self):
        count = 0
        MSE = 0
        clean_fold('/mnt/A/meteorological/2500_ref_seq/multi_scale_gan/')
        for i in range(self.test_data_itertor.number_sample):
            frame_dat, names, file_name, = self.test_data_itertor.sample_process(i)
            frame_dat[frame_dat > 80] = 0
            frame_dat[frame_dat < 15] = 0
            frame_dat = normalize_frames(frame_dat)
            clean_fold('/mnt/A/meteorological/2500_ref_seq/multi_scale_gan/' + file_name + '/')
            ...
            for j in range(len(frame_dat) - 23):
                ...
                clean_fold(
                    '/mnt/A/meteorological/2500_ref_seq/multi_scale_gan/' + file_name + '/process_' + str(j) + '/')
                clean_fold('/mnt/A/meteorological/2500_ref_seq/multi_scale_gan/' + file_name + '/process_' + str(
                    j) + '/predict/')
                for img_index in range(len(current_process) - 4):
                    if img_index < 10:
                        ...
                        break
                    imsave('/mnt/A/meteorological/2500_ref_seq/multi_scale_gan/' + file_name + '/process_' + str(
...
def main():
    ...
    load_path = '../model_lib/multi_scale_gan/model.ckpt'
