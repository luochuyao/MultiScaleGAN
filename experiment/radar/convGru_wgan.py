...
class SequenceModel(SequenceBaseModel):
    ...
    def __init__(self,
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.d_rms = tf.train.RMSPropOptimizer(learning_rate=self.info['MODEL_PARAMETER_D']['LEARNING_RATE_D']) \
                .minimize(self.d_loss, var_list=self.dis_vars)
            self.g_rms = tf.train.RMSPropOptimizer(learning_rate=self.info['MODEL_PARAMETER_G']['LEARNING_RATE_G']) \
                .minimize(self.g_loss, var_list=self.gen_vars)
            self.g_adam = tf.train.AdamOptimizer(learning_rate=learning_rate_init)\
                .minimize(self.loss, var_list=self.gen_vars)

        gpu_options = tf.GPUOptions(allow_growth=True)
        self.d_clip = [v.assign(tf.clip_by_value(v, -0.01, 0.01)) for v in self.dis_vars]
...
if __name__ == '__main__':
    ...
    for idx, cell in enumerate(configuration['GENERATOR_MODEL_NETS']['CELLS']):
        ...
        if idx == len(configuration['GENERATOR_MODEL_NETS']['CELLS']) - 1:
            net = Conv2DGRU(
                name='generator_conv_gru_layer_' + str(idx),
